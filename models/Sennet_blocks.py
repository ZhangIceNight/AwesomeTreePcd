...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Sen-net's blocks are based on Hugues THOMAS's KPConv version in 06/03/2020
#
...
from models.Sennet_kernels import load_kernels
...
def block_decider(block_name,
    ...
    elif block_name in ['SAmodule_pct']:
        return SAmodule_pct(block_name, in_dim, out_dim, radius, layer_ind, config)

    elif block_name in ['ESAttention']:
        return Sattention(block_name, in_dim, out_dim, radius, layer_ind, config)

    elif block_name in ['SAmodule_Vector_Attention']:
        return SAmodule_Vector_Attention(block_name, in_dim, out_dim, radius, layer_ind, config)

    elif block_name == 'max_pool' or block_name == 'max_pool_wide':
        return MaxPoolBlock(layer_ind)

    elif block_name == 'max_pool_2':
        return MaxPoolBlock2(layer_ind)
...
class MaxPoolBlock2(nn.Module):

    def __init__(self, layer_ind):
        """
        Initialize a max pooling block with its ReLU and BatchNorm.
        """
        super(MaxPoolBlock2, self).__init__()
        self.layer_ind = layer_ind
        return
...
class SAmodule(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        ...
        self.q_conv = nn.Conv1d(out_dim * 2, out_dim * 2, 1, bias=False)
        self.k_conv = nn.Conv1d(out_dim * 2, out_dim * 2, 1, bias=False)
        #self.q_conv.weight = self.k_conv.weight
        self.v_conv = nn.Conv1d(out_dim * 2, out_dim * 2 , 1, bias=False)
...
class SAmodule_pct(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        ...
        :param config: parameters
        """
        super(SAmodule_pct, self).__init__()  #
        ...
        self.q_conv = nn.Conv1d(out_dim , out_dim , 1, bias=False)
        self.k_conv = nn.Conv1d(out_dim , out_dim , 1, bias=False)

        self.v_conv = nn.Conv1d(out_dim , out_dim  , 1, bias=False)




        self.batch_norm_conv = BatchNormBlock(out_dim , self.use_bn, self.bn_momentum)
        self.softmax = nn.Softmax(dim=-1)
        # Second upscaling mlp
        self.unary2 = UnaryBlock(out_dim , out_dim, self.use_bn, self.bn_momentum)
        self.trans_conv1 = nn.Conv1d(out_dim, out_dim, 1, bias=False)
        self.trans_conv2 = nn.Conv1d(out_dim, out_dim, 1, bias=False)
    ...
    def forward(self, features, batch):
        ...
        x = self.unary1(features)
        # self-attention
        x = x.unsqueeze(2)  # n*c*b
        x = x.transpose(0, 2) # b，c，n

        x_q = self.q_conv(x).permute(0, 2, 1)  # b*n*c

        x_k = self.k_conv(x)  #  b*c*n

        x_v = self.v_conv(x)  # this is value
        ...
        x = x.unsqueeze(2)
        x = x.transpose(0, 2)
        x = self.leaky_relu(self.after_norm(self.trans_conv2(self.trans_conv1(x))))
        x = x.transpose(0, 2)
        x = x.squeeze()
...
class SAmodule_Vector_Attention(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        ...
        """
        super(SAmodule_Vector_Attention, self).__init__()
        ...
        self.v_conv = nn.Conv1d(out_dim, out_dim, 1, bias=False)
        self.bn_1d = nn.BatchNorm1d(out_dim)
        self.tans_conv1 = nn.Conv1d(out_dim, out_dim, 1, bias=False)
        self.tans_conv2 = nn.Conv1d(out_dim, out_dim, 1, bias=False)
        self.tans_conv3 = nn.Conv1d(out_dim, out_dim, 1, bias=False)
    ...
    def forward(self, features, batch):
        ...
        x_q = self.q_conv(x).permute(0, 1, 2)  # b*n*c

        x_k = self.k_conv(x)  #  b*c*n   this is key

        x_v = self.v_conv(x)  # b*c*n


        energy = x_q - x_k #  b, n, n
        energy = self.leaky_relu(self.tans_conv2(self.tans_conv1(energy)))
        attention = self.softmax(energy)
        attention = attention / (1e-9 + attention.sum(dim=1, keepdims=True))
        ...
        x_v = x_v.squeeze()
        x = torch.mul(attention, x_v)
        x = features - x
        x = x.unsqueeze(2)
        x = x.transpose(0, 2)
        x = self.leaky_relu(self.bn_1d(self.tans_conv3(x)))
        x = x.transpose(2, 1)
        x = x.squeeze()
...
class SharedMLPBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, use_bn, bn_momentum, no_relu=False):
        ...
        Initialize a shared-weight MLP block with its ReLU and BatchNorm.
        :param in_dim: dimension input features
        :param out_dim: dimension input features
        :param use_bn: boolean indicating if we use Batch Norm
        :param bn_momentum: Batch norm momentum
    ...
    def __repr__(self):
        return 'SharedMLPBlock(in_feat: {:d}, out_feat: {:d}, BN: {:s}, ReLU: {:s})'.format(self.in_dim,
                                                                                        self.out_dim,
                                                                                        str(self.use_bn),
                                                                                        str(not self.no_relu))
...
class Sattention(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        ...
        self.q_conv = SharedMLPBlock(out_dim , out_dim ,self.use_bn, self.bn_momentum)
        self.k_conv = SharedMLPBlock(out_dim , out_dim ,self.use_bn, self.bn_momentum)
        self.v_conv = SharedMLPBlock(out_dim , out_dim, self.use_bn, self.bn_momentum)
        self.mu_1 = SharedMLPBlock(out_dim , out_dim, self.use_bn, self.bn_momentum)
        self.mu_2 = SharedMLPBlock(out_dim , out_dim, self.use_bn, self.bn_momentum)
    ...
    def forward(self, features, batch):
        ...
        x_q = self.q_conv(features)

        x_k = self.k_conv(features)
        x_k = x_k.transpose(0, 1)

        x_v = self.v_conv(features)
        ...
        attention = self.softmax(energy)
        attention = attention / (1e-9 + attention.sum(dim=1, keepdims=True))
        x = attention @ x_v  # n*c * n*n = c*n


        x = self.leaky_relu(self.after_norm(x))


        memory_v = self.mu_2(features)
        memory_energy = self.mu_1(x_q)
        memory_attention = self.softmax(memory_energy)
        memory_attention = memory_attention / (1e-9 + attention.sum(dim=1, keepdims=True))
        memory_out = torch.mul(memory_attention, memory_v)
        memory_out = self.leaky_relu(self.after_norm(memory_out))
        ...
        return self.leaky_relu(x + shortcut + memory_out)